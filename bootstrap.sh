...
download_packages_1400() {
  #-- SST 14.0.0
  $GETCMD $SSTCORE_14_0_0 $SSTCORE_14_0_0_URL >> /dev/null 2>&1
  $GETCMD $SSTELEMENTS_14_0_0 $SSTELEMENTS_14_0_0_URL >> /dev/null 2>&1

  if test -f "$SSTCORE_14_0_0"; then
    echo "Downloaded $SSTCORE_14_0_0"
  else
    echo "Failed to download $SSTCORE_14_0_0"
    exit -1
  fi

  if test -f "$SSTELEMENTS_14_0_0"; then
    echo "Downloaded $SSTELEMENTS_14_0_0"
  else
    echo "Failed to download $SSTELEMENTS_14_0_0"
    exit -1
  fi
}

download_packages_1410() {
  #-- SST 14.1.0
  $GETCMD $SSTCORE_14_1_0 $SSTCORE_14_1_0_URL >> /dev/null 2>&1
  $GETCMD $SSTELEMENTS_14_1_0 $SSTELEMENTS_14_1_0_URL >> /dev/null 2>&1

  if test -f "$SSTCORE_14_1_0"; then
    echo "Downloaded $SSTCORE_14_1_0"
  else
    echo "Failed to download $SSTCORE_14_1_0"
    exit -1
  fi

  if test -f "$SSTELEMENTS_14_1_0"; then
    echo "Downloaded $SSTELEMENTS_14_1_0"
  else
    echo "Failed to download $SSTELEMENTS_14_1_0"
    exit -1
  fi
}
...
verify_packages_11() {
  ...
  if [ "$SSTELEMENTS_SHA1" = "$SSTELEMENTS_11_0_0_SHA1" ]; then
    ...
    echo "FAILED TO VERIFY SSTELEMENTS 11 SHA1 HASH"
    exit -1
  fi
...
verify_packages_111() {
  ...
  if [ "$SSTCORE_SHA1" = "$SSTCORE_11_1_0_SHA1" ]; then
    ...
    echo "FAILED TO VERIFY SSTELEMENTS 11.1.0 SHA1 HASH"
    exit -1
  fi
...
verify_packages_1201() {
  ...
  if [ "$SSTELEMENTS_SHA1" = "$SSTELEMENTS_12_0_1_SHA1" ]; then
    ...
    echo "FAILED TO VERIFY SSTELEMENTS 12.0.1 SHA1 HASH"
    exit -1
  fi
...
verify_packages_1210() {
  ...
  if [ "$SSTELEMENTS_SHA1" = "$SSTELEMENTS_12_1_0_SHA1" ]; then
    ...
    echo "FAILED TO VERIFY SSTELEMENTS 12.1.0 SHA1 HASH"
    exit -1
  fi
...
verify_packages_1300() {
  ...
  if [ "$SSTELEMENTS_SHA1" = "$SSTELEMENTS_13_0_0_SHA1" ]; then
    ...
    echo "FAILED TO VERIFY SSTELEMENTS 13.0.0 SHA1 HASH"
    exit -1
  fi
...
verify_packages_1310() {
  ...
  if [ "$SSTELEMENTS_SHA1" = "$SSTELEMENTS_13_1_0_SHA1" ]; then
    ...
    echo "FAILED TO VERIFY SSTELEMENTS 13.1.0 SHA1 HASH"
    exit -1
  fi

}

verify_packages_1400() {
  #-- get the sha1 of the package
  SSTCORE_SHA1=`$SHA1SUM $SSTCORE_14_0_0 | awk '{print $1}'`
  SSTELEMENTS_SHA1=`$SHA1SUM $SSTELEMENTS_14_0_0 | awk '{print $1}'`

  if [ "$SSTCORE_SHA1" = "$SSTCORE_14_0_0_SHA1" ]; then
    echo "SSTCORE 14.0.0 SHA1 VERIFIED!"
  else
    echo "FAILED TO VERIFY SSTCORE 14.0.0 SHA1 HASH"
    exit -1
  fi

  if [ "$SSTELEMENTS_SHA1" = "$SSTELEMENTS_14_0_0_SHA1" ]; then
    echo "SSTELEMENTS 14.0.0 SHA1 VERIFIED!"
  else
    echo "FAILED TO VERIFY SSTELEMENTS 14.0.0 SHA1 HASH"
    exit -1
  fi
...
verify_packages_1410() {
  #-- get the sha1 of the package
  SSTCORE_SHA1=`$SHA1SUM $SSTCORE_14_1_0 | awk '{print $1}'`
  SSTELEMENTS_SHA1=`$SHA1SUM $SSTELEMENTS_14_1_0 | awk '{print $1}'`

  if [ "$SSTCORE_SHA1" = "$SSTCORE_14_1_0_SHA1" ]; then
    echo "SSTCORE 14.1.0 SHA1 VERIFIED!"
  else
    echo "FAILED TO VERIFY SSTCORE 14.1.0 SHA1 HASH"
    exit -1
  fi

  if [ "$SSTELEMENTS_SHA1" = "$SSTELEMENTS_14_1_0_SHA1" ]; then
    echo "SSTELEMENTS 14.1.0 SHA1 VERIFIED!"
  else
    echo "FAILED TO VERIFY SSTELEMENTS 14.1.0 SHA1 HASH"
    exit -1
  fi
}
...
setup_symlinks_1400() {
  $MY_LN -fs $MY_PWD/$SSTCORE_14_0_0 ./containers/singularity/$SSTCORE_14_0_0
  $MY_LN -fs $MY_PWD/$SSTELEMENTS_14_0_0 ./containers/singularity/$SSTELEMENTS_14_0_0
}

setup_symlinks_1410() {
  $MY_LN -fs $MY_PWD/$SSTCORE_14_1_0 ./containers/singularity/$SSTCORE_14_1_0
  $MY_LN -fs $MY_PWD/$SSTELEMENTS_14_1_0 ./containers/singularity/$SSTELEMENTS_14_1_0
}
